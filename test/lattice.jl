...
@testset "Test creating `Lattice`s from 6 lattice constants" begin
    ...
    @test all(
        primitivevectors(Lattice(1, 2, 3, 75, 40, 81; axis = :c)) .≈
    ...
    # Compared with Python's results
    @test all(
        primitivevectors(Lattice(3, 4, 20, 45, 90, 126; axis = :c)) .≈ (
    ...
    @test all(
        primitivevectors(Lattice(2, 2, 3, 90, 90, 90; axis = :c)) .≈
        ([2, 0, 0], [0, 2, 0], [0, 0, 3]),
    )  # Tetragonal
    # From https://github.com/LaurentRDC/crystals/blob/7c544fe/crystals/tests/test_lattice.py#L165
    # Compared with Python's results
    @test all(
        primitivevectors(Lattice(1, 1, 1, 87, 87, 87; axis = :c)) .≈
    ...
    # Compared with Python's results
    @test all(
        primitivevectors(Lattice(1, 2, 3, 90, 115, 90; axis = :c)) .≈
    ...
    # Compared with Python's results
    @test all(
        primitivevectors(Lattice(2, 3, 1, 115, 90, 90; axis = :c)) .≈
    ...
    # Compared with Python's results
    @test all(
        primitivevectors(Lattice(3, 1, 2, 90, 90, 115; axis = :c)) .≈
    ...
    # Compared with Python's results
    @test all(
        primitivevectors(Lattice(2, 2, 3, 90, 90, 120; axis = :c)) .≈
    ...
    # Compared with Python's results
    @test all(
        primitivevectors(Lattice(3, 2, 2, 120, 90, 90; axis = :c)) .≈
    ...
    # Compared with Python's results
    @test all(
        primitivevectors(Lattice(2, 3, 2, 90, 120, 90; axis = :c)) .≈
    ...
    # Compared with Python's results
    @test all(
        primitivevectors(Lattice(2, 2, 2, 90, 120, 90; axis = :c)) .≈
