...
julia> pkg"dev CrystallographyBase"
```

in Julia REPL.

Then the package will be cloned to your local machine at a path. On macOS, by default is
located at `~/.julia/dev/CrystallographyBase` unless you modify the `JULIA_DEPOT_PATH`
...
[here](https://mineralscloud.github.io/CrystallographyBase.jl/dev/), but there are cases
